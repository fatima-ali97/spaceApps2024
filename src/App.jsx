...
import { Chart as ChartJS, CategoryScale, LinearScale, BarElement, Title, Tooltip, Legend, PointElement, LineElement, plugins } from 'chart.js';
...
const LineOptions = {
  ...
  return (
    <>
    <h1>Dataset name: Pilot top-down CO2 Budget constrained by the v10 OCO-2 MIP Version 1.0</h1>
      <select name="country" id="country" value={country} onChange={(event) => setCountry(event.target.value)}>
      ...
      <br />
      <select name="experiment" id="experiment" value={experiment} onChange={(event) => setExperiment(event.target.value)}>
        <option value="ALL">ALL</option>
      ...
      <div>
                        ...
                        <tr>
                            {Object.keys(data[0]).map((key, index) => {
                              if(experiment === "ALL") {
                                ...
                                if(words.includes(experiment) || key === "Alpha 3 Code" || key === "Year") {
                                  return (
                                    <th key={index}>{key}</th>
                                  );
                                }
                      ...
                      <tr key={rowIndex}>
                        {
                          Object.entries(row).map(([property, value], colIndex) => {
                            if(experiment === "ALL") {
                              ...
                              if (words.includes(experiment) || property === "Alpha 3 Code" || property === "Year") {
        ...
        <div>
          ...
          {
            (country === "ALL")? <h2>Please select a country to display charts</h2> : 
            <div>
